...
export class ReservationSettingsPage implements HasSideMenu , IHaveTitle {
    private URL : string;
    private page: Page;
    ...
    constructor(page: Page, config: ConfigType) {
        ...
        this.URL  = config.envUrl + '/expo/' + config.expoId + '/settings/reservation';
    }
    async areWeOnThisPage(): Promise<boolean> {
        const title = await this.page.title();
        return title.includes('Reservation settings - ExpoFP');
    }
