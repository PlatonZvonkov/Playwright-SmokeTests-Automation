...
export class TeamThisEventSettingsPage implements HasSideMenu , IHaveTitle {
    private URL : string;
    private page: Page;
    ...
    constructor(page: Page, config: ConfigType) {
        ...
        this.URL  = config.envUrl + '/expo/'+ config.expoId + '/settings/teammates';
    }
    async areWeOnThisPage(): Promise<boolean> {
        const title = await this.page.title();
        return title.includes('Team - this event settings - ExpoFP');
    }
