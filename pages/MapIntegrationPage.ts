...
export class MapIntegrationPage implements HasSideMenu , IHaveTitle {
    private URL : string;
    private page: Page;
    ...
    constructor(page: Page, config: ConfigType) {
        ...
        this.URL  = config.envUrl + '/expo/integrations/options/'+ config.expoId;
    }
    async areWeOnThisPage(): Promise<boolean> {
        const title = await this.page.title();       
        return title.includes('Map integration - ExpoFP');
    }
