...
export class ReservationDiscountsPage implements HasSideMenu , IHaveTitle {
    private URL : string;
    private page: Page;
    ...
    constructor(page: Page, config: ConfigType) {
        ...
        this.URL  = config.envUrl + '/settings/discounts/'+ config.expoId;
    }
    async areWeOnThisPage(): Promise<boolean> {
        const title = await this.page.title();
        return title === 'Discounts - ExpoFP';
    }
